...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return new NextResponse(body, { headers: { "Content-Type": material.mimeType, "Content-Disposition": `${inline ? "inline" : "attachment"}; filename="${material.fileName.replace(/[\\\"]/g, "_")}"`, "Cache-Control": "private, no-store" } });
}
